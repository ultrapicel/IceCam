...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION = "v9.3.2-noise-filter-surface-map";
    ...
    private void dashboard(){
        ...
        addBtn(w,"5 · Open target camera manually", diagStep==5, v->{diagStep=6; show("Open Camera / Telegram / Chrome camera screen now. Keep each camera open for 5–10 seconds. Then return and press Step 6 Export Lite Debug Bundle."); renderUi();});
        ...
        n.addView(muted("v9.3.2 required markers: target [LOAD], openCamera, createCaptureSession, CaptureRequest target/surface ownership traces"));
    }
    ...
    private void hooksTab(){
        LinearLayout c=card(); section(c,"Hook configuration", "v9.3.2 maps CaptureRequest targets, Surface ownership and session lifecycle with self-noise filtering. Camera frames are not replaced yet."); pillRow(c,"Mode",mode); pillRow(c,"Target",cameraMode); pillRow(c,"Compatibility",compatibilityMode);
        ...
        addBtn(m,"strict-real", compatibilityMode.equals("strict-real"),v->{compatibilityMode="strict-real";writeAppConfigViaRoot();renderUi();});
        addBtn(m,"compatibility", compatibilityMode.equals("compatibility"),v->{compatibilityMode="compatibility";writeAppConfigViaRoot();renderUi();});
    ...
    private String json(){ String uri=(mediaUri==null?"":mediaUri.toString()).replace("\\","\\\\").replace("\"","\\\""); return "{\n"+
            ...
            "  \"pipelineStage\": \"surface-ownership-noise-filter\",\n"+
