...
public class IceCamHook implements IXposedHookLoadPackage {
    ...
    private void hookCamera2(final XC_LoadPackage.LoadPackageParam lp){
        try{
            ...
            XposedHelpers.findAndHookMethod(cls,"getCameraIdList",new XC_MethodHook(){
                @Override protected void beforeHookedMethod(MethodHookParam p){log("[Camera2] getCameraIdList before pkg="+lp.packageName+" mode="+mode());}
            ...
            XposedHelpers.findAndHookMethod(cls,"getCameraCharacteristics",String.class,new XC_MethodHook(){
                @Override protected void beforeHookedMethod(MethodHookParam p){log("[Camera2] getCameraCharacteristics pkg="+lp.packageName+" id="+p.args[0]);}
            });
            for(Method m:cls.getDeclaredMethods()){
                ...
                XposedBridge.hookMethod(m,new XC_MethodHook(){
                    @Override protected void beforeHookedMethod(MethodHookParam p){
                        ...
                        String md=mode();
                        log("[Camera2] openCamera pkg="+lp.packageName+" id="+id+" mode="+md);
                        if("block-open-test".equals(md))p.setResult(null);
                    }
                });
            }
    ...
    private void hookCamera1(final XC_LoadPackage.LoadPackageParam lp){
        try{
            ...
            XposedHelpers.findAndHookMethod(cam,"open",int.class,new XC_MethodHook(){
                @Override protected void beforeHookedMethod(MethodHookParam p){log("[Camera1] open pkg="+lp.packageName+" id="+p.args[0]+" mode="+mode());}
            });
            XposedHelpers.findAndHookMethod(cam,"open",new XC_MethodHook(){
                @Override protected void beforeHookedMethod(MethodHookParam p){log("[Camera1] open default pkg="+lp.packageName+" mode="+mode());}
            });
    ...
    private static String readConfig(){try{File f=new File(CONFIG); if(!f.exists())return"{}"; FileInputStream fis=new FileInputStream(f); ByteArrayOutputStream bos=new ByteArrayOutputStream(); byte[] buf=new byte[4096]; int n; while((n=fis.read(buf))>0)bos.write(buf,0,n); fis.close(); return bos.toString();}catch(Throwable t){return"{}";}}
